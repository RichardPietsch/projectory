...
app.post('/api/auth/forgot-password', async (req, res) => {
  const forgotPasswordRateLimitMax = Number(process.env.AUTH_FORGOT_PASSWORD_RATE_LIMIT_MAX || 10);
  const forgotPasswordRateLimitWindowMs = Number(process.env.AUTH_FORGOT_PASSWORD_RATE_LIMIT_WINDOW_MS || 60000);
  const forgotPasswordNow = Date.now();
  const forgotPasswordIpKey = String(req.ip || req.socket?.remoteAddress || 'unknown');
  const forgotPasswordBucketKey = `auth-forgot-password|${forgotPasswordIpKey}`;
  const forgotPasswordBucket = routeRateLimitBuckets.get(forgotPasswordBucketKey);

  if (!forgotPasswordBucket || forgotPasswordNow - forgotPasswordBucket.windowStart >= forgotPasswordRateLimitWindowMs) {
    routeRateLimitBuckets.set(forgotPasswordBucketKey, { windowStart: forgotPasswordNow, count: 1 });
  } else {
    forgotPasswordBucket.count += 1;
    if (forgotPasswordBucket.count > forgotPasswordRateLimitMax) {
      const retryAfterMs = Math.max(1, forgotPasswordRateLimitWindowMs - (forgotPasswordNow - forgotPasswordBucket.windowStart));
      res.setHeader('Retry-After', String(toRetryAfterSeconds(retryAfterMs)));
