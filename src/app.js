...
function forgotPasswordRouteRateLimitMiddleware(req, res, next) {
  const max = Number(process.env.AUTH_FORGOT_PASSWORD_RATE_LIMIT_MAX || 10);
  const windowMs = Number(process.env.AUTH_FORGOT_PASSWORD_RATE_LIMIT_WINDOW_MS || 60000);
  const now = Date.now();
  const ipKey = String(req.ip || req.socket?.remoteAddress || 'unknown');
  const bucketKey = `auth-forgot-password|${ipKey}`;
...
function spaShellRouteRateLimitMiddleware(req, res, next) {
  const max = Number(process.env.SPA_SHELL_RATE_LIMIT_MAX || 240);
  const windowMs = Number(process.env.SPA_SHELL_RATE_LIMIT_WINDOW_MS || 60000);
  const now = Date.now();
  const ipKey = String(req.ip || req.socket?.remoteAddress || 'unknown');
  const bucketKey = `spa-shell|${ipKey}`;
  const bucket = routeRateLimitBuckets.get(bucketKey);
