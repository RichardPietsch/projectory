(function registerAdminProjectsView(globalScope) {
  function renderAdministrationProjectsView({ state }) {
    ...
    const rows = sortedProjects
      .map(
        (project) => `<tr class="border-t border-slate-800">
              ...
              <td class="p-2">
                <button class="rounded border border-slate-600 px-2 py-1 text-xs" onclick='openAdminProjectModal(${JSON.stringify(project)})'>Edit</button>
    ...
    return `<div class="rounded-xl border border-slate-800 bg-slate-900 p-4"><div class="mb-3 flex items-center justify-between"><h2 class="text-lg font-semibold">Projects</h2><button class="rounded bg-emerald-600 px-3 py-2 text-sm font-semibold" onclick="openAdminProjectModal()">Add Project</button></div><table class="w-full text-left text-sm"><thead><tr class="text-slate-400"><th class="p-2"><button class="inline-flex items-center gap-1 hover:text-slate-100" onclick="setAdminProjectsSortField('project')">Project ${state.adminProjectsSort.startsWith('project_') ? (state.adminProjectsSort.endsWith('_asc') ? '↑' : '↓') : ''}</button></th><th class="p-2"><button class="inline-flex items-center gap-1 hover:text-slate-100" onclick="setAdminProjectsSortField('dates')">Dates ${state.adminProjectsSort.startsWith('dates_') ? (state.adminProjectsSort.endsWith('_asc') ? '↑' : '↓') : ''}</button></th><th class="p-2"><button class="inline-flex items-center gap-1 hover:text-slate-100" onclick="setAdminProjectsSortField('budget')">Budget ${state.adminProjectsSort.startsWith('budget_') ? (state.adminProjectsSort.endsWith('_asc') ? '↑' : '↓') : ''}</button></th><th class="p-2">Actions</th></tr></thead><tbody>${rows}</tbody></table></div>`;
  }
