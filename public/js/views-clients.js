(function registerClientsView(globalScope) {
  function renderClientsView({ state, renderPriorityPill }) {
    ...
    return `<div class="rounded-xl border border-slate-800 bg-slate-900 p-4">
              <div class="mb-3 flex items-center justify-between"><h2 class="text-lg font-semibold">${t('entity.clients')}</h2><button class="rounded bg-[#00d8ff] text-slate-950 px-3 py-2 text-sm font-semibold" onclick="openAdminClientModal()">${t('clients.add')}</button></div>
